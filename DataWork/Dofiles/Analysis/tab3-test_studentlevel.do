...
* --------------------------------------------------------------------------- */
	...
	* Initiate final LaTeX file
	file open test  using "${master_tab}/test_studentlevel.tex", 		 ///
		 text write replace
	...
	* Remove spaces
	filefilter "${master_tab}/test_studentlevel.tex"		/// 
			   "${master_tab}/tab3-test_studentlevel.tex"	///
			   , from("[1em]") to("") replace
	erase 	   "${master_tab}/test_studentlevel.tex"
	
	* Add link to the file (filefilter does not provide it automatically)
	di as text `"Open final file in LaTeX here: {browse "${master_tab}/tab3-test_studentlevel.tex":${master_tab}/tab3-test_studentlevel.tex}"'
