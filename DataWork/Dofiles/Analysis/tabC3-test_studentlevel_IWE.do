...
********************************************************************************
	...
	** CREATES:	   		Table C3: Impact on Student Learning - Interaction-Weighted Estimator (IWE)
						"${master_tab}/tabC3-test_studentlevel_IWE.tex"
...
* --------------------------------------------------------------------------- */
	...
	* Store regressions results with standardized test scores
	foreach 	 subject 		in media MT LT CH CN {
		...
		preserve
		keep if !mi(prof_`subject') & !mi(school_treated) & !mi(strata)
		// This doesn't solve the issue
		
		* Save estimates of regression on the overall sample (using only z-scores)
		cap noisily 																	///
		eststo  `subject'_all	 	   : GSSUtest prof_`subject' school_treated strata,	///
										 cluster(inep)
		...
		* Same estimates by grade
		foreach   				 grade  in 5 6 1 {
			...
			* Define type of "ensino"
			...
			cap noisily 																		///
			eststo `subject'_grade`grade'	 : 	GSSUtest prof_`subject' school_treated strata	///
	...
	* Save separate sample regression results in a unique LaTeX file for the paper	
	foreach sample in grade5 grade6 {
	...
	* Initiate final LaTeX file
	file open  test											///
		 using "${master_tab}/test_studentlevel_IWE.tex"	///
		 , text write replace
	...
	* Remove spaces
	filefilter "${master_tab}/test_studentlevel_IWE.tex"		/// 
			   "${master_tab}/tabC3-test_studentlevel_IWE.tex"	///
			   , from("[1em]") to("") replace
	erase 	   "${master_tab}/test_studentlevel_IWE.tex"
	
	* Add link to the file (filefilter does not provide it automatically)
	di as text `"Open final file in LaTeX here: {browse "${master_tab}/tabC3-test_studentlevel_IWE.tex":${master_tab}/tabC3-test_studentlevel_IWE.tex}"'
