...
********************************************************************************
	...
	** CREATES:	   		Table C2: Impact on Student Learning - Blocked Difference-in-Means
						"${master_tab}/tabC2-test_studentlevel_DIM.tex"
...
* --------------------------------------------------------------------------- */
	...
	* Store regressions results with standardized test scores
	foreach 	 subject 		in media MT LT CH CN {
		...
		* Save estimates of regression on the overall sample (using only z-scores)
		eststo  `subject'_all	 	   : blockdim prof_`subject',						 ///
										 treat(school_treated) strata(strata)			 ///
									     cl(inep)
				
		sum     proficiencia_`subject' if !mi(prof_`subject') & school_treated == 0
		...
		* Same estimates by grade
		foreach   				 grade  in 5 6 1 {
			...
			* Define type of "ensino"
			...
			eststo `subject'_grade`grade'	 : 	blockdim prof_`subject'					  ///
	...
	* Initiate final LaTeX file
	file open  test											///
		 using "${master_tab}/test_studentlevel_DIM.tex"	///
		 , text write replace
	...
	* Remove spaces
	filefilter "${master_tab}/test_studentlevel_DIM.tex"		/// 
			   "${master_tab}/tabC2-test_studentlevel_DIM.tex"	///
			   , from("[1em]") to("") replace
	erase 	   "${master_tab}/test_studentlevel_DIM.tex"
	
	* Add link to the file (filefilter does not provide it automatically"
	di 			""
	di as text `"Open final file in LaTeX here: {browse "${master_tab}/tabC2-test_studentlevel_DIM.tex":${master_tab}/tabC2-test_studentlevel_DIM.tex}"'
