...
********************************************************************************
	...
	** CREATES:	   		Table 8: Impact on Socio-Emotional Skills
						"${master_tab}/tab8-socio_studentlevel.tex"
...
* ---------------------------------------------------------------------------- */
	...
	* Store regressions results recoded for conscientiousness vignette
	foreach 	skill  in 		   agreeab consc extrav neurot openness {
		...
		* Save estimates of regression on the overall sample
		eststo `skill'_all		 : reghdfe z_`skill'_c school_treated, abs(strata) cl(inep)
	...
	* Initiate final LaTeX file
	file open   socio									///
		 using "${master_tab}/socio_studentlevel.tex"	///
	   , text write replace
	...
	* Remove spaces
	filefilter "${master_tab}/socio_studentlevel.tex"		/// 
			   "${master_tab}/tab8-socio_studentlevel.tex"	///
			   , from("[1em]") to("") replace
	sleep  		${sleep}
	erase 	   "${master_tab}/socio_studentlevel.tex"
	
	* Add link to the file (filefilter does not provide it automatically)
	di as text `"Open final file in LaTeX here: {browse "${master_tab}/tab8-socio_studentlevel.tex":${master_tab}/tab8-socio_studentlevel.tex}"'
